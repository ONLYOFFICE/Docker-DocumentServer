...
COPY wait_for_db.sh /tmp/wait_for_db.sh

ENV CONNECTION_STR="/opt/mssql-tools/bin/sqlcmd -S localhost,1433 -U SA -P "$MSSQL_SA_PASSWORD" -C -Q"

RUN echo -e "#!/bin/bash\n/opt/mssql/bin/sqlservr &" > /tmp/run_mssql.sh && \
    bash /tmp/run_mssql.sh && \
    bash /tmp/wait_for_db.sh && \
